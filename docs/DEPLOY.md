...
## 📡 采集源现状与已知限制（2026-07 修复后）

系统每天自动去 **FDA / EMA / NMPA** 三个官方源搬「药械组合（Combination Products）」相关法规。下面说明每个源现在的情况，方便您看懂日志和看板数字：

- **FDA（主源，已修复 ✅）**：主源是「美国联邦公报 Federal Register API」，用 14 个组合产品相关检索词（`combination product`、`drug-device`、`prefilled syringe`、`auto-injector`、`drug-eluting`、`transdermal`、`co-packaged`、`cross-labeled` 等）检索近 365 天文档。之前有个 bug：FR 返回的文档虽然真和组合产品相关，但系统末尾的「关键词二次确认」只看了标题/摘要片段，把 20 条全误杀了（日志显示「FDA 采集 20 条，校验过滤丢弃 20 条，入库 0 条」）。现已修复——FR 既然已按关键词检索命中，就如实把命中词带入内容，四重校验（发布日期 / 近期窗口 / 导航页 / 真实文档）照常全部执行，真实法规得以入库。**FDA 是看板新增条目的主要来源。**
  - 补充：FDA 组合产品办公室（OCP）指导原则页面上的文件多为历史文档（早于 365 天窗口），故不会被自动采集；历史上的经典指导原则已包含在 9 条精选种子里。

- **EMA（源地址已修复 ✅，但多数时候入库 0 条属正常）**：原来的两个 RSS 地址（`/en/news/rss.xml`、`/en/rss-feed/medicines-human/chronological.xml`）已失效返回 404，已替换为官方可用的 `news.xml` 与 `whats-new.xml`（已实测可访问、返回合法条目）。EMA 官方新闻多为通用监管动态，标题/正文里带「combination product / 组合产品」字样的条目比较少，所以**大多数采集运行里 EMA 经四重校验后入库数为 0，这是正确行为，不代表源坏了**。一旦 EMA 发布组合产品相关新闻，会被自动捕获进看板。

- **NMPA（可达，但近期少有组合产品条目，常采集 0 条属正常）**：英文镜像（`english.nmpa.gov.cn`）可以正常访问，会返回带发布日期的文章；但近期栏目里很少出现「药械组合 / 组合产品」关键词的条目，因此常被正确过滤为 0 条。中文栏目（药审中心 cde、器审中心 cmde）可能受网站防护（WAF）拦截。NMPA 相关经典文件已包含在 9 条精选种子里，新增以 FDA 实时采集为主。

> 一句话总结：看板条目能不能稳定超过 9 条，主要看 **FDA** 近期有没有发布组合产品法规——通常有。EMA / NMPA 是否新增，取决于它们近期是否发布了带组合产品关键词的内容，不是每次都有，属正常。
